...
const LoginForm: React.FC<Props> = ({ onSwitch }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (code === 'auth/invalid-email-format' || code === 'auth/invalid-email') {
        ...
        setUnverified(true);
        setBannerErr('Your email is not verified. Check your inbox for the verification link.');
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      await signOut(auth);
      setResendSent(true); setBannerErr(''); setUnverified(false);
    } catch {
      setBannerErr('Could not resend. Please check your email and password.');
    } finally {
